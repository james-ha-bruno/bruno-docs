...
**Symptoms:** SSO redirects to Entra ID successfully, user authenticates, but Bruno shows "SSO Login failed" or "You do not have necessary permissions". Decoded SAML response has no `groups` or `roles` attribute in the `<AttributeStatement>`.
...
1. **No group claim configured**
   ...
   - **Fix:** Follow the [Group Claims configuration guide](./configure-saml-sso-with-entra-id#alternative-using-entra-id-group-claims)
...
7. Look for the `<AttributeStatement>` section — verify a `groups` or `roles` attribute is present with the expected group names
